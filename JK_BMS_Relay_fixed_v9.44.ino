/*
 * ESP32 JK-BMS BLE 中继网关  (修复版 v9.44, S3/C3 通用)
...
struct GatewayConfig {
  ...
  char blePin[8] = "1234";
  int cellInfoOffset = 32;  // ★v9.44: 0x02 电芯帧字段偏移 (可配置, 默认 32)
};
...
// ★v9.40/v9.43: MTU 低时的重试计数 —— 防止"MTU<100 无限断开重试导致永远连不上":
//   Server 侧客户端持续活跃时 MTU 协商可能每次都被挤超时, 若每次都断开重试 → 死循环无数据。
//   重试 MTU_MAX_RETRY 次仍低则降级接受 MTU=23 继续工作 (有数据优于无数据,
//   且下次 520 重连时 Server 安静仍有机会协商到 303 自愈)。v9.43 起统一双平台, 不再区分 S3/C3。
...
void parseCellInfo(const uint8_t* data, size_t len) {
  ...
  // ★v9.44: 偏移可配置 (默认 32, Web 后台可改)。不同极空固件/串数的 0x02 帧字段偏移可能不同
  //   (用户 24 串 BMS 实测 +32 布局; 其他版本可能是 +0 或其它值) → 换 BMS 时后台调整, 免重编译。
  const size_t off = g_config.cellInfoOffset;
...
void handlePostConfig() {
  ...
  // ★v9.44: 电芯帧偏移可配置 (0~64, 偶数), 换不同布局 BMS 时调整, 立即生效
  if (doc.containsKey("cellInfoOffset")) {
  ...
  resp["cellInfoOffset"] = g_config.cellInfoOffset;  // ★v9.44
...
"<span class='app-title' id='appTitle'>JK BMS 蓝牙中继后台<sub class='app-version'>v9.44</sub></span>\n"
"</div>\n"
"</div>\n"
...
"<div class='config-label'>电芯帧偏移</div>\n"
"<div style='display:flex;gap:6px'>\n"
"<input type='number' class='config-input' id='cellInfoOffset' min='0' max='64' step='2' value='32'>\n"
"<button class='btn btn-save' onclick='saveOffset()'>保存偏移</button>\n"
"</div>\n"
"<div style='font-size:11px;color:#999;margin-top:4px'>数据字段偏移(默认32)。换不同布局BMS后Web显示错乱时调整: 0/16/32/48/64</div>\n"
"</div>\n"
"</div>\n"
"<div style='flex:1'>\n"
...
void initBleServer() {
  ...
  //   (原 pAdv->start() 会在 BMS 连上前开 5s 可连窗口, 移除)
  // pAdv->start();
...
bool connectToBms() {
  ...
  // ★v9.43: MTU 检查统一 (去掉 S3/C3 平台分支) —— 前提是 v9.41/v9.42 隐身方案:
  //   BMS 连接期间 Server 侧完全安静 (不广播 + 断开旧客户端), MTU 协商不再被挤,
  //   理论上每次都能稳定协商到 303, 无需按平台区分。
  //   逻辑: MTU<100 (协商异常) → 有限重试 MTU_MAX_RETRY 次 → 仍低则降级接受
  //   MTU=23 继续 (有数据优于无数据; 下次 520 重连时可再协商 303 自愈)。
  if (pClient->getMTU() < 100) {
    if (g_mtuRetryCount < MTU_MAX_RETRY) {
      ...
      g_bmsScanFailCount++;  // ★v9.33: 计入退避, 避免立刻重连再次阻塞 web
      if (g_bmsScanFailCount > 4) g_bmsScanFailCount = 4;
      pClient->disconnect();
      NimBLEDevice::deleteClient(pClient);  // ★v9.34: FIX-22 同款, 失败即销毁
      pClient = nullptr;
      return false;
    }
    // 降级接受: 重试已达上限, 继续用 MTU=23 (有数据优于永远连不上)
    g_mtuRetryCount = 0;  // 复位, 下次 520 重连时重新给 2 次机会
    Serial.printf("[BMS] MTU=%u 重试%d次仍低, 降级接受继续 (520 重连时可再协商)\n",
      pClient->getMTU(), MTU_MAX_RETRY);
  } else {
    g_mtuRetryCount = 0;  // 协商成功 (MTU>=100), 复位计数
  }
  ...
  //   安全: 此处在 loop 上下文 (connectToBms 由 loop 调用), 非 NimBLE 事件回调,
...
void loadConfig() {
  ...
  // ★v9.44: 读取电芯帧偏移 (默认 32, 兼容用户 24 串 BMS 的 +32 布局)
  int savedOffset = prefs.getInt("cellInfoOffset", 32);
  if (savedOffset >= 0 && savedOffset <= 64) {
    g_config.cellInfoOffset = savedOffset;
  }
  prefs.end();
...
void setup() {
  ...
  Serial.println();
  Serial.println("========================================");
  Serial.println(" ESP32 JK-BMS BLE 中继网关 (修复版 v9.44, S3/C3 通用)");
  Serial.println("========================================");
...
void loop() {
  ...
  if (pBleServer && millis() - lastAdvCheck > 5000) {
    ...
    //   挤占 BMS 连接资源导致 MTU 协商失败 (MTU=23) / reason 520 (用户实测: MTU=23 都是
    //   客户端在 BMS 未连接时先连上中继导致的)。
    //   机制: 停广播 = 客户端扫不到中继 → 无法发起连接 → BMS 连接时 Server 侧安静,
    //   MTU 能正常协商到 303; BMS 连上后恢复广播, 客户端即可正常连接取数。
    //   副作用 (可接受): BMS 连上前的窗口内客户端显示"找不到设备"。
    if (!bmsConnected) {
      if (pAdv->isAdvertising()) {
        Serial.println("[BLE-Server] BMS 未连接, 停止广播 (防客户端抢连)");
        pAdv->stop();
      }
      // ★v9.42: BMS 未连接时还主动断开已连接的客户端 —— 仅停广播不会断开已建立的连接,
      //   旧客户端仍占用 Server 侧连接槽 + 事件调度 → BMS 重连时 MTU 协商照样被挤 (MTU=23/520 循环)。
      //   断开后客户端重新扫描, 但广播已停扫不到 → 等 BMS 连上广播恢复再连 (全链路隐身)。
      //   安全: 此处 loop 上下文, 非 NimBLE 事件回调, disconnect 安全。
      for (int i = 0; i < MAX_BLE_CLIENTS; i++) {
        if (g_connStates[i].active) {
          ...
          // unregisterConn 由 onDisconnect 回调执行 (disconnect 异步触发)
        }
      }
    } else {
      // BMS 已连接: 恢复原逻辑 (未达上限保持广播)
