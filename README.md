...
`02-ESP32固件/esp32s3-gateway.ino`（v2.12），Arduino IDE 编译，所需库：
...
- **自愈机制**：卡死看门狗 30s 重启 / DTU 串口无响应 8 分钟重启 / MQTT 下行静默断 12 分钟重启
...
- **里程学习**：充满(98%+)→骑行→再充电自动结算，实测修正里程基数；学习状态与历史记录在设置页实时显示
